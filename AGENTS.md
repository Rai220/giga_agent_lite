...
- **Тест-кейсы хранятся в отдельном файле** (`tests/test-cases.md`). Перед коммитом агент обязан прогнать все релевантные тест-кейсы через UI.
- **Юнит-тесты** хранятся в директории `tests/` и запускаются командой `npm run test`. Юнит-тесты покрывают логику инструментов, парсинг, утилиты и другие модули, не требующие браузерного окружения.

## Процесс коммита в master

Агент может коммитить в `master` **только после успешной проверки**:

1. Прогнать юнит-тесты: `npm run test` — все тесты должны пройти.
2. Проверка типов: `npm run typecheck` — без ошибок.
3. Прогнать UI тест-кейсы из `tests/test-cases.md` — все релевантные сценарии должны работать корректно в браузере.
4. Только после успешного прохождения всех трёх шагов агент делает коммит в `master`.
